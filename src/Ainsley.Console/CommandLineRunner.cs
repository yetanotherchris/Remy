    ...
    public class CommandLineRunner
    {
        ...
        private static Uri ParseConfigPath(string configPath, ILogger logger)
        {
            ...
            try
            {
                ...
                if (uri.IsAbsoluteUri && !File.Exists(uri.ToString()))
                {
                    logger.Error($"The Yaml configuration '{uri}' does not exist");
                    return null;
                }
            ...
            {
                logger.Error($"The Yaml configuration '{configPath}' is not a valid path or url.");
                return null;
            }
        }
    }
